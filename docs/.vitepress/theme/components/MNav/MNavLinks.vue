...
<template>
  <section class="m-nav-section" role="region" :aria-labelledby="formatTitle">
    <h2 v-if="title" :id="formatTitle" class="nav-title" tabindex="-1" role="heading" aria-level="2">
      {{ title }}
      <a class="header-anchor" :href="`#${formatTitle}`" :aria-label="`链接到 ${title} 部分`" role="link"></a>
    </h2>
    <div class="m-nav-links" role="list" :style="{ '--grid-columns': gridColumns }">
      <MNavLink v-for="item in itemsWithKeys" :key="item.uniqueKey" :noIcon="noIcon" v-bind="item" role="listitem" />
    </div>
  </section>
</template>
